...
from utils_training import filter_annotation_features, evaluate_model, custom_batch_generator
...
for model_name, config in model_configurations.items():
    ...
    train_generator = custom_batch_generator(X_train_seq, X_train_anno, Y_train_filtered, batch_size, config, pad_symbol)
    validation_generator = custom_batch_generator(X_test_seq, X_test_anno, Y_test_filtered, batch_size, config, pad_symbol)

    train_steps_per_epoch = np.ceil(len(X_train_seq) / batch_size)
    validation_steps_per_epoch = np.ceil(len(X_test_seq) / batch_size)  
