...
# exon length gets scaled to the same amount of nucleotides for each gene and the coverage normalized 
...
import pandas as pd
import math
from plot_genes import plot_genes_scaled
...
for feature in gtffile:
	if feature.type == "gene":
		...
		coverage_dict[gene_name] = numpy.zeros((N_BAM, ((exonsize + 2* winwidth))+4), dtype='f') # + 4 to also store exon length and position info (direction, start, end) #exon_length + 2*...
...
for j,file in enumerate(bamlist):
	...
	for i, p in enumerate(exonpos):
		...
		if p[0].pos<p[1].pos: # positive strand
			...
			upstream = HTSeq.GenomicInterval( p[0].chrom, p[0].pos - (winwidth), p[0].pos, "." ) # p[0].chrom, p[0].pos - winwidth, p[0].pos, "."
			downstream = HTSeq.GenomicInterval( p[0].chrom, p[1].pos, p[1].pos + (winwidth), "." ) # p[0].chrom, p[1].pos, p[1].pos + winwidth, "."
			coverage_dict[gene_names[i]][j,1] = p[0].pos - (winwidth)
			coverage_dict[gene_names[i]][j,2] = p[1].pos + (winwidth)
			...
			downstream = HTSeq.GenomicInterval( p[0].chrom, p[1].pos - (winwidth) + 1, p[1].pos+1, "." ) # p[0].chrom, p[1].pos - winwidth+1, p[1].pos+1, "."
			upstream = HTSeq.GenomicInterval( p[0].chrom, p[0].pos+1, p[0].pos + (winwidth) + 1,  "." ) # p[0].chrom, p[0].pos+1, p[0].pos + winwidth +1,  "."
			coverage_dict[gene_names[i]][j,1] = p[1].pos - (winwidth) + 1
			coverage_dict[gene_names[i]][j,2] = p[0].pos + (winwidth) + 1
		...
		exon_length=(exon_length-1)/exonsize # increment to be used for loop 
		l=0 # loop iterators
		m=0
		while m < exonsize:
			if round(l)!=round(l+exon_length):
				exoncvg_scaled[m]=numpy.mean(exoncvg[int(round(l)):int(round(l+exon_length))])
			else:
				exoncvg_scaled[m]=exoncvg[int(round(l))]
			l += exon_length
			m += 1	
		if p[0].pos<p[1].pos:
			up_wincvg = numpy.fromiter(coverage[upstream], dtype='f', count = int((winwidth)))
			wincvg = numpy.concatenate((up_wincvg, exoncvg_scaled), axis = 0)  
			wincvg = numpy.concatenate((wincvg, numpy.fromiter(coverage[downstream], dtype='f', count= int((winwidth)))), axis = 0)
		else:
			wincvg = numpy.concatenate((numpy.fromiter(coverage[downstream], dtype='f', count= int((winwidth))), exoncvg_scaled), axis = 0)  
...
index_of_max = max(range(len(combined_list)), key=lambda i: len(combined_list[i]))

column_names = ['Gene_name','Exon_length','Start_pos','End_pos','Strand'] + [f'position_{i}_coverage' for i in range(1, len(combined_list[index_of_max]) - 4)]
combined_list = [column_names] + combined_list

# Define the CSV file path
csv_file = outdir+"/gene_coverage_scaled_"+str(winwidth)+"_"+str(exonsize)+".csv"
...
print(plot_genes_scaled(csv_file,outdir, winwidth, exonsize, plots))
