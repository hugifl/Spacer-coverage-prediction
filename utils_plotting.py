...
def plot_predicted_vs_observed(model, model_name, X_test_seq, X_test_annot, normalized_coverage_with_windows_info, no_plots, no_bin, outpath, dataset_name, window_size, promoter_df, terminator_df, gene_df, binsize):
...
def plot_window_coverage_normalized_compare_profiles(normalized_coverage_with_windows_info_1, normalized_coverage_with_windows_info_2, experiment_1_name, experiment_2_name , no_plots, no_bin, outpath, window_size, promoter_df, terminator_df, gene_df, binsize, random = True):
    ...
    for idx in indices:
        ...
        normalized_coverage_2 = normalized_coverage_with_windows_info_2[matching_index[0], 2:]
        gene_starts = []
        gene_ends = []
        terminator_starts = []
        terminator_ends = []
        ...
        # Plotting¨
        ymax = max(normalized_coverage_1.max() + (normalized_coverage_1.max() * 0.1), normalized_coverage_2.max() + (normalized_coverage_2.max() * 0.1))
        pyplot.style.use('ggplot')
        pyplot.figure(figsize=(12, 6))
        pyplot.plot(x_coord, normalized_coverage_1, color="blue", label=experiment_1_name)
        pyplot.plot(x_coord, normalized_coverage_2, color="orange", label=experiment_2_name)
        pyplot.title(f"Normalized coverage over window: {window_start}-{window_end}")
        pyplot.xlabel('Bins')
        ...
        for _, terminator_row in terminator_df.iterrows():
            ...
            if (0 <= terminator_start < window_size) or (0 <= terminator_end < window_size) or ((terminator_start < window_size) and (terminator_end > window_size)):
                if ((terminator_start < window_size) and (terminator_end > window_size)):
                    ...
                    pyplot.text(label_x_position, 2.5*gene_y, terminator_name, color='red', fontsize=8)
        pyplot.legend()
        pyplot.savefig(outpath+"_"+str(window_start)+"_"+str(window_end)+"_window_coverage_" + experiment_1_name +"_"  + experiment_2_name +".png")
        pyplot.close()
    return "Plots generated"