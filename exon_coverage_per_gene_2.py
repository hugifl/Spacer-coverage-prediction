...
# dimension of coverage vector is the different for each gene and corresponds to the length upstream-exon-downstream 
...
for feature in gtffile:
	if feature.type == "gene":
		...
		coverage_dict[gene_name] = numpy.zeros((N_BAM, ((exon_length + 2* winwidth))+4), dtype='f') # + 4 to also store exon length and position info (direction, start, end) #exon_length + 2*...
...
for j,file in enumerate(bamlist):
	...
	for i, p in enumerate(exonpos):
		...
		if p[0].pos<p[1].pos:
			...
			wincvg = numpy.concatenate((numpy.fromiter(coverage[downstream], dtype='f', count= int((winwidth))), exoncvg), axis = 0)  
...
csv_file = outdir+"/gene_coverage_variable._"+str(winwidth)+"_"+str(exonsize)+".csv"
...
print(plot_genes(csv_file,outdir, winwidth, exonsize, plots, False))