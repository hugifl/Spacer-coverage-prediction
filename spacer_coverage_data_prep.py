...
if normalization_unit == 'gene':
    print("normalizing for gene expression")
...
if normalization_unit == 'TU':
    print("normalizing for gene expression on TU level")
    # Normalize coverage for gene expression (RPKM) values per gene to remove effects of differential gene expression.
    low_expressed_genes, TU_spacer_counts_normalized_df = get_normalized_spacer_counts_per_TU(count_df, gene_perc, count_dict)       
    coverage_array_summed = coverage_df_summed.to_numpy()
    coverage_array_TU_normalized = normalize_coverage_per_TU(coverage_array_summed, TU_spacer_counts_normalized_df, no_bin, binsize)
    print("normalize for total aligned reads to make scale comparable across experiments")
    # Normalize coverage for total aligned reads to make scale comparable across experiments
    coverage_array_TU_and_library_size_normalized = normalize_coverage_for_tot_aligned_reads(coverage_array_TU_normalized, total_aligned_reads, expected_aligned_reads_per_experiment)
    coverage_df_TU_and_library_size_normalized = pd.DataFrame(coverage_array_TU_and_library_size_normalized)
    coverage_df_TU_and_library_size_normalized.columns = coverage_df_summed.columns
    #coverage_df_summed = filter_windows(coverage_df_summed, low_expressed_genes, gene_df)   # If I integrate multiple datasets, this wouldnt work
    print("saving data")
    # Save data frame
    TU_spacer_counts_normalized_df.to_csv(outdir+'gene_spacer_counts.csv', index=False)
    coverage_df_TU_and_library_size_normalized.to_csv(outdir+'window_coverage_data_summed.csv', index=False)
    with open(outdir+'tot_number_aligned_reads.txt', 'w') as file:
        # Write the integer to file
        file.write(str(total_aligned_reads))
